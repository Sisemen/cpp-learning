...
namespace Day1_DesignPatterns
{
  ...
  std::string HealthComponent::Save() const
  {
    return "<HealthComponent> " + std::to_string(health) + " " + std::to_string(maxHealth);
  }

  void HealthComponent::Load(const std::string& data)
  {
    sscanf(data.c_str(), "%d %d", &health, &maxHealth);
  }
  ...
  std::string MovementComponent::Save() const
  {
    return "<MovementComponent> " + std::to_string(x) + " " + std::to_string(y) + " " + std::to_string(speed);
  }

  void MovementComponent::Load(const std::string& data)
  {
    sscanf(data.c_str(), "%f %f %f", &x, &y, &speed);
  }
  ...
  std::string DamageComponent::Save() const
  {
    return "<DamageComponent> " + std::to_string(attackPower);
  }

  void DamageComponent::Load(const std::string& data)
  {
    sscanf(data.c_str(), "%d", &attackPower);
  }

  WeaponComponent::WeaponComponent(int dg, int rng) : damage(dg), range(rng) {}

  void WeaponComponent::Update(float dt)
  {
    // Could implement weapon logic here
  }

  std::string WeaponComponent::Save() const
  {
    return "<WeaponComponent> " + std::to_string(damage) + " " + std::to_string(range);
  }

  void WeaponComponent::Load(const std::string& data)
  {
    sscanf(data.c_str(), "%d %d", &damage, &range);
  }
  ...
  {
    std::string data = "\t<Entity> " + name + "\n";
    data += "\t\tComponents: " + std::to_string(components.size()) + "\n";
  ...
  void Entity::Load(const std::string& data)
  {
    // Simple example: just set the name (real implementation would parse components too)
