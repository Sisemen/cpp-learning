...
public:
  Enemy(const std::string& n, int lvl)
    : name(n), health(lvl * 20), level(lvl)
  {
...
public:
  ...
  bool Use()
  {
    if (quantity <= 0)
    {
      std::cout << "No " << name << " is remaining." << std::endl;
      return false;
    }
   
    quantity--;
    std::cout << "Used one " << name << ". Remaining: " << quantity << std::endl;
...
private:
  ...
  void Healup()
  {
    if (playerItem && playerItem->Use())
    {
      health += 10;
      std::cout << name << " healed for 10 HP. Health: " << health << std::endl;
    }
  }
};
...
void DoGameEntitySystem()
{
  ...
  auto item = std::make_unique<Item>("HealingPotion", 3);
  ...
  for (int i = 0; i < 3; i++)
  {
    enemies.emplace_back("Troll", i + 1);
  }

  enemies[0].TakeDamage(20);
  enemies[1].TakeDamage(15);
  enemies[2].TakeDamage(10);
